...
welcome_msg = ["Que a força esteja com você", "Bem vindo", "Você é um mito", "Okaerinasai",
               "Esta pessoa é muito boa no Websexo ->", "Você é um baitola", "Você é corno", "Você é gay"]
try:
...
while (Sair == False):
    ...
    print(
        f'{C}[{Y}*{C}]Patrocinadores: {CY}Josuke(Douglas){CO}, {CY}Margarina{CO}, {CY}♞⃟𝜣͢𝑭𝑵–M4ŘŤÏŇ§♞⃟🇱🇰LÍDER{CO} e {CY}N 3 X 0{CO}')
    print(
    ...
    if op == '6' or op == '06':
        def cpfcon():
            ...
            try:
                r = json.loads(
                    requests.get('http://poisonbr.sytes.net:12344/ConsultaCPF/' + cpf).content.decode('utf-8'))
                print(f'''
                	{C}CPF: {Y}{r["cpf"]}
